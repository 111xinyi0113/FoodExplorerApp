    ...
    const newPost = {
      ...
      try {
        ...
        const updatedPosts = existingPosts.map(p => {
          if (p.date === route.params.post.date) {
            return { ...p, postText, postImages, postLocation };
          }
          return p;
        });
        ...
        Alert.alert('Error', 'Failed to update the post: ' + e.message);
      }
    }, [postText, postImages, postLocation, navigation, route.params]);
